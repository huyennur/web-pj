...
majors = ["CN1", "CN2", "CN3", "CN4", "CN5", "CN6", "CN7", "CN8", "CN9", "CN10", "CN11"]
def list(request):
    ...
    return render(
            request, "list.html", {'students': students, 'grades': grades, 'addresses': addresses, 'currentAddress': currentAddress, 'majors': majors, 'jobs': jobs}
        )
...
def search(request):
    ...
    if address == 'Nước ngoài':
        students = Student.objects.filter(school__Grade__icontains=grade).filter(MSSV__in=Job.objects.filter(Major__icontains=major).values_list('MSSV')).exclude(MSSV__in=Address.objects.filter(CurrentAddress__in=addresses).values_list('MSSV'))
    else:
        students = Student.objects.filter(school__Grade__icontains=grade).filter(MSSV__in=Job.objects.filter(Major__icontains=major).values_list('MSSV')).filter(MSSV__in=Address.objects.filter(CurrentAddress__icontains=address).values_list('MSSV'))
...
def add(request):
    if request.method == "POST":
        ...
        School(Student=user.student, MSSV=MSSV, StartTimeShool=StartTimeShool, FinishTimeSchool=FinishTimeSchool,
               Grade=Grade, Class=Class, Achievement=None).save()
